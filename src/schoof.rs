...
pub fn schoof(a: &BigInt, b: &BigInt, q: &BigInt) {
    for l in &vec![BigInt::from(3), BigInt::from(5)] {
        ...
        let b_den1 = (division_polynomial::psi(a, b, &ql).power_i(2))
        ...
        if found {
            ...
            let d = (division_polynomial::omega(a, b, &ql) - 
                    UnitBuilder::new().coef_i(1).ypow(&q.power_i(2)).finalize().to_pol())
                  * division_polynomial::psi(a, b, &ql).power_i(3);
            let d = d.modular(q);
            ...
            let num1 = &d * (e * f.power_i(2) - d.clone().power_i(2)) -
                UnitBuilder::new().coef_i(1).ypow(&q.power_i(2)).finalize()
                .to_pol() * 
                division_polynomial::psi(a, b, &ql).power_i(3) * f.power_i(3);
            ...
            let den1 = division_polynomial::psi(a, b, &ql).power_i(3) * f.power_i(3) * division_polynomial::psi(a, b, &ql);
            ...
            let den2 = division_polynomial::psi(a, b, &jj).to_frob(q).power_i(3);
            println!("den2:{} {}", line!(), den2.to_string());
            ...
            let p3: Polynomial;
            if p2.has_y() {
                p3 = p2 / UnitBuilder::new().coef_i(1).ypow_i(1).finalize().to_pol();
            } else {
                p3 = p2.clone();
            }
            ...
            if !found {
                ...
                // (e) x
                let p1 = UnitBuilder::new().coef_i(1).xpow(&q.clone()).finalize().to_pol()
                ...
                if !p4.is_zero() {
                    ...
                    // (e) y
                    let p5 = (UnitBuilder::new().coef_i(1).ypow(&a.clone()).finalize()
                              .to_pol()
