...
pub fn psi(a: &BigInt, b: &BigInt, n: &BigInt, p: &BigInt) -> Polynomial {
    assert!(*n >= Zero::zero());
    if *n == Zero::zero() {
        ...
        let g = psi(a, b, &(m.clone()-2), p);
        let h = psi(a, b, &(m.clone()+1), p).power(&BigInt::from(2));
        let i = psi(a, b, &m.clone(), p) * (e*f - g*h);
        let j = i / UnitBuilder::new().coef(2).ypow(1).finalize().to_pol();
        j.ec_reduction(a, b, p)
    }
}

pub fn phi(a: &BigInt, b: &BigInt, n: &BigInt, p: &BigInt) -> Polynomial {
    assert!(*n >= One::one());
    let i = UnitBuilder::new().coef(1).xpow(1).finalize().to_pol() * psi(a, b, &(n.clone()), p).power(&BigInt::from(2))
              - psi(a, b, &(n.clone() + 1), p) * psi(a, b, &(n.clone()-1), p);
    i.ec_reduction(a, b, p)
}

pub fn omega(a: &BigInt, b: &BigInt, n: &BigInt, p: &BigInt) -> Polynomial {
    assert!(*n >= One::one());
    if *n == One::one() {
        UnitBuilder::new().coef(1).ypow(1).finalize().to_pol()
    } else {
        let i = (psi(a, b, &(n.clone()+2), p) * psi(a, b, &(n.clone()-1), p).power(&BigInt::from(2))
                     - psi(a, b, &(n.clone()-2), p) * psi(a, b, &(n.clone()+1), p).power(&BigInt::from(2)))
                  / UnitBuilder::new().coef(4).ypow(1).finalize().to_pol();
        i.ec_reduction(a, b, p)
    }
}
...
fn division_polynomial_test() {
    // psi
    let psi0 = psi(&BigInt::from(1), &BigInt::from(1), &BigInt::from(0), &Zero::zero());
    assert_eq!(psi0.to_string(), "0");

    let psi1 = psi(&BigInt::from(1), &BigInt::from(1), &BigInt::from(1), &Zero::zero());
    assert_eq!(psi1.to_string(), "1");

    let psi2 = psi(&BigInt::from(1), &BigInt::from(1), &BigInt::from(2), &Zero::zero());
    assert_eq!(psi2.to_string(), "2 y");

    let psi3 = psi(&BigInt::from(1), &BigInt::from(1), &BigInt::from(3), &Zero::zero());
    assert_eq!(psi3.to_string(), "3 x^4 + 6 x^2 + 12 x - 1");

    let psi4 = psi(&BigInt::from(1), &BigInt::from(1), &BigInt::from(4), &Zero::zero());
    assert_eq!(psi4.to_string(), "4 x^6 y + 20 x^4 y + 80 x^3 y - 20 x^2 y - 16 x y - 36 y");

    let psi5 = psi(&BigInt::from(1), &BigInt::from(1), &BigInt::from(5), &Zero::zero());
    assert_eq!(psi5.to_string(), "5 x^12 + 62 x^10 + 380 x^9 - 105 x^8 + 240 x^7 - 540 x^6 - 696 x^5 - 2045 x^4 - 1680 x^3 - 290 x^2 - 740 x - 287");

    // phi
    let phi1 = phi(&BigInt::from(1), &BigInt::from(1), &BigInt::from(1), &Zero::zero());
    assert_eq!(phi1.to_string(), "x");

    let phi2 = phi(&BigInt::from(1), &BigInt::from(1), &BigInt::from(2), &Zero::zero());
    assert_eq!(phi2.to_string(), "x^4 - 2 x^2 - 8 x + 1");
    
    let phi3 = phi(&BigInt::from(1), &BigInt::from(1), &BigInt::from(3), &Zero::zero());
    assert_eq!(phi3.to_string(), "x^9 - 12 x^7 - 96 x^6 + 30 x^5 - 24 x^4 + 84 x^3 + 48 x^2 + 105 x + 72");

    // omega
    let omega1 = omega(&BigInt::from(1), &BigInt::from(1), &BigInt::from(1), &Zero::zero());
    assert_eq!(omega1.to_string(), "y");

    let omega2 = omega(&BigInt::from(1), &BigInt::from(1), &BigInt::from(2), &Zero::zero());
