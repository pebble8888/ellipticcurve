...
pub fn psi(a: &BigInt, b: &BigInt, n: i32) -> Polynomial {
    ...
    } else if n.mod_floor(&2) == One::one() {
        let m = (n - 1).div_floor(&2);
        assert!(m < n);
        let e = psi(a, b, m+2);
        let f = psi(a, b, m).power(3);
        let g = psi(a, b, m-1);
        let h = psi(a, b, m+1).power(3);
        (e*f - g*h).reduction(a, b)
    } else {
        let m = n.div_floor(&2);
        assert!(m < n);
        let e = psi(a, b, m+2);
        let f = psi(a, b, m-1).power(2);
        let g = psi(a, b, m-2);
        let h = psi(a, b, m+1).power(2);
        let i = psi(a, b, m) * (e*f - g*h);
...
pub fn omega(a: &BigInt, b: &BigInt, n: i32) -> Polynomial {
    ...
    if n == One::one() {
        ...
        let i = (psi(a, b, n+2) * psi(a, b, n-1).power(2)
                     - psi(a, b, n-2) * psi(a, b, n+1).power(2))
                  / TermBuilder::new().coef(4).ypow(1).build();
        i.reduction(a, b)
    }
}
...
fn division_polynomial_test_psi() {
    // psi
    let psi0 = psi(&BigInt::from(1), &BigInt::from(1), 0);
    assert_eq_str!(psi0, "0");

    let psi1 = psi(&BigInt::from(1), &BigInt::from(1), 1);
    assert_eq_str!(psi1, "1");

    let psi2 = psi(&BigInt::from(1), &BigInt::from(1), 2);
    assert_eq_str!(psi2, "2 y");

    let psi3 = psi(&BigInt::from(1), &BigInt::from(1), 3);
    assert_eq_str!(psi3, "3 x^4 + 6 x^2 + 12 x - 1");

    let psi4 = psi(&BigInt::from(1), &BigInt::from(1), 4);
    assert_eq_str!(psi4, "4 x^6 y + 20 x^4 y + 80 x^3 y - 20 x^2 y - 16 x y - 36 y");

    let psi5 = psi(&BigInt::from(1), &BigInt::from(1), 5);
...
fn division_polynomial_test_phi() {
    // phi
    let phi1 = phi(&BigInt::from(1), &BigInt::from(1), 1);
    assert_eq_str!(phi1, "x");

    let phi2 = phi(&BigInt::from(1), &BigInt::from(1), 2);
    assert_eq_str!(phi2, "x^4 - 2 x^2 - 8 x + 1");
    
    let phi3 = phi(&BigInt::from(1), &BigInt::from(1), 3);
...
fn division_polynomial_test_omega() {
    // omega
    let omega1 = omega(&BigInt::from(1), &BigInt::from(1), 1);
    assert_eq_str!(omega1, "y");

    let omega2 = omega(&BigInt::from(1), &BigInt::from(1), 2);
    assert_eq_str!(omega2, "x^6 + 5 x^4 + 20 x^3 - 5 x^2 - 4 x - 9");

    let omega3 = omega(&BigInt::from(1), &BigInt::from(1), 3);
    ...
    let omega4 = omega(&BigInt::from(1), &BigInt::from(1), 4);
    ...
    let omega5 = omega(&BigInt::from(1), &BigInt::from(1), 5);
